%-----------------------------------------------------------------------------
% Template for AIMS-Cameroon essay phase 2015-2016, ver 1, 160416
%
...
\usepackage{fancyhdr} %For header image on title page
...
\title{\vspace*{-4cm} Estimating the Causal Effect of Insecticide-Treated Net Use on Malaria Infection in Kenya: Comparing Cluster-Specific and Population-Average Effects Using Traditional and Machine Learning Methods}
\author{Nicholas Mwanza (nicholas.mwanza@aims-cameroon.org)\\
African Institute for Mathematical Sciences (AIMS)\\
Cameroon\\\\
{\small Supervised by: Dr. Haisa Osmanli}\\
{\small University of Regina, Canada}%
}
\date{{\small 30 May 2026}\\%
  {\scriptsize\it Submitted in Partial Fulfillment of 
    a Structured Masters Degree at AIMS-Cameroon}
}
%-------------------------------------------------------------------------
\begin{document}
\pagestyle{empty}
%The pagestyle 'empty' is used by \maketitle for the 'report' document class. 
% In order to include a header image on the first page, the pagestyle 'empty' 
% must be redefined. However, the redefinition should only be local which 
% is why we group the commands below within braces
{ 
\setlength{\headheight}{95pt} %Ensure that the header image fits within the header.
% Without this line, fancyhdr will increase \headheight to fit the header image. 
% In addition, the increased \headheight will be used throughout the document.
% There will thus be a large empty space at the top of every page...
\fancypagestyle{empty}{%
  \fancyhf{}% Clear header/footer
  \renewcommand{\headrulewidth}{0pt} %Remove the horizontal line
  \fancyhead[C]{\includegraphics[width = \textwidth]{Figures/aimsHeader.png}}% Logo
} %end \fancypagestyle
...
% the conclusion. It does not include the acknowledgements or references.
% 
...
% See the example in chapter three for defining that. Be *consistent*
...
%-----------------------------------------------------------------------------
% Uncomment the next four lines if you have an appendix
% Appendices are usually not counted, and are only allowed if you
% speak to the Academic Director about the need for it. Get approval first,
...
% We use the 'plain' bibliography style by default here.  
...
% style than plain, then you may change to that style.
\renewcommand{\bibname}{References}
%\nocite{*} %Uncomment this line if you want all references in the bib file to appear in the bibliography (i.e. even the non-cited references). This is not advisable and must be approved by the Academic Director.
